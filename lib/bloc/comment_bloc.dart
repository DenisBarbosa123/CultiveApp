...
class CommentBloc extends BlocBase{
  ...
  Future<Comentario> saveComment(int userId, int postId, String commentBody, String token) async {
    Comentario comentario = Comentario();
    comentario.corpo = commentBody;
    try{
      Response response = await Dio()
          .post(PathConstants.createComment(userId, postId),
  ...
  Future<Comentario> editComment(int userId, int postId, String commentBody, String token, int commentId) async {
    Comentario comentario = Comentario();
    comentario.corpo = commentBody;
    try{
      Response response = await Dio()
          .put(PathConstants.editComment(userId, postId, commentId),
          options: RequestOptions(headers: {'Authorization':token}),
          data: comentario.toJson());
      if(response.statusCode == 200){
  ...
  Future<void> deleteComment(int userId, int postId, String token, int commentId) async {
    try{
