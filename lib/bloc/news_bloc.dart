...
class NewsBloc extends BlocBase{
  int page = 1;
  List<News> news = [];
  StreamController<List<News>> _streamController = StreamController<List<News>>();
  ...
  void getListNews() async {
    ...
    Response response = await Dio().get("https://newsapi.org/v2/everything?q=agro&language=pt&pageSize=10&page=$page&apiKey=$API_KEY");
    if(response.statusCode == 200){
      debugPrint("Everything ok with News API response");
      page ++;
      var jsonDecoded = response.data;
      List<News> newsList = jsonDecoded["articles"].map<News>(
              (map){
