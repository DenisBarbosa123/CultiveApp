...
class WeatherBloc extends BlocBase{
  ...
  void getUpdateWeather() async {
    ...
    Response response = await Dio().get(_getUrl(position.latitude.toString(), position.longitude.toString()));
    Weather weather = Weather.fromJson(response.data);
    _streamController.add(weather);
  }

  _getUrl(String lat, String log) => "https://api.hgbrasil.com/weather?fields=only_results, temp, condition_slug, city, humidity, wind_speedy, forecast, date, weekday, max, min, description, condition&key=695a3e17&lat=$lat&log=$log&user_ip=remote";
